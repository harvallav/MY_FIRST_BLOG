...
This code groups the coffee_ratings dataset by country_of_origin and calculates the average values for various coffee attributes, including total_cup_points, aroma, flavor, aftertaste, acidity, and body, while ignoring any missing values. The resulting summarized data is stored in the coffee_modified variable.

```{r}

coffee_modified <- coffee_ratings %>% 
...
```{r}
coffee_mod <- coffee_modified %>%
  group_by(country_of_origin) %>%
  summarize(
    total_points_mean = mean(total_cup_points, na.rm = TRUE),
    aroma_mean = mean(aroma, na.rm = TRUE),
    flavor_mean = mean(flavor, na.rm = TRUE),
    aftertaste_mean = mean(aftertaste, na.rm = TRUE),
    acidity_mean = mean(acidity, na.rm = TRUE),
    body_mean = mean(body, na.rm = TRUE)
  ) %>%
  arrange(desc(total_points_mean)) %>%
  head(5)
...
The code groups the coffee_modified dataset by country_of_origin and calculates the average scores for several coffee attributes, such as total cup points, aroma, and flavor. It then sorts the results in descending order based on the average total cup points and selects the top 5 countries. The final summary is stored in coffee_mod.

phew , lets make the other changes as well like we need them .

```{r}
coffee_modc <- coffee_mod %>%
  mutate(
  ...
  select(total_points_mean, aroma_rank, flavor_rank, aftertaste_rank, acidity_rank, body_rank)
...
coffee_modc <- coffee_mod %>%
  mutate(
    total_points_rank = rank(total_points_mean, ties.method = "first"),  # Change here
    aroma_rank = rank(aroma_mean, ties.method = "first"),                # Change here
    flavor_rank = rank(flavor_mean, ties.method = "first"),              # Change here
    aftertaste_rank = rank(aftertaste_mean, ties.method = "first"),      # Change here
    acidity_rank = rank(acidity_mean, ties.method = "first"),            # Change here
    body_rank = rank(body_mean, ties.method = "first")                   # Change here
  ) %>%
  select(total_points_mean, aroma_rank, flavor_rank, aftertaste_rank, acidity_rank, body_rank)
...
I dont know why this is reverse here and not 1-5. I tried asking chatgpt, but I will leave it here as it really isnt bothering my graphs and data.

The reason you see only 5 rows in the coffee_mod dataset is due to the head(5) function, which limits the output to the top 5 entries after sorting by the average total cup points(in the previous one). The conversion from means to ranks provides a clear, ordinal representation of how each country's coffee attributes compare relative to each other, making it easier to identify which countries perform better or worse in each category.

### now building the graph

I had created alot of wrong graphs to get this one right and I did not have group=country which gave me a graph with just dots and multiple errors.
...
coffee_ratings %>%
  ggplot(aes(x = attribute, y = rank, color = country,group=country)) +
  geom_bump(size = 2) +  # Add lines for rank progression
  geom_point(size = 6, shape = 21) +  # Add points at each rank
  xlab("Coffee Attribute") +
  ylab("Rank") +
  scale_color_manual(values = c("Ethiopia" = "#00AFBB", 
                                "Japan" = "#E7B800", 
                                "Kenya" = "#FC4E07", 
