...
gf_histogram(~Score,
  ...
  gf_vline(xintercept = mean_score, color = "red") %>%
  gf_labs(
    title = "Histograms of Grades",
    x = "(out of 10)", y = "Count"
  ) %>%
  gf_text(7 ~ (mean_score + 0.5),
    label = "Overall Mean"
  ) %>%
...
filtered_data <- grades %>%
  filter(Degree %in% c("B.Des", "B.FA"))

# Specify and calculate the test statistic
result <- filtered_data %>%
  specify(Score ~ Degree) %>%
  hypothesise(null = "point", mu = 0) %>%  # Specify a point hypothesis for the means
  calculate(stat = "t", order = c("B.FA", "B.Des")) # Order to ensure B.Des is compared against B.FA
